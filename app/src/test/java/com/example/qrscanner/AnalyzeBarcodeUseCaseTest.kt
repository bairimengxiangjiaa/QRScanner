...
class AnalyzeBarcodeUseCaseTest {
    ...
    private val useCase = AnalyzeBarcodeUseCase(ValidateUrlUseCase())
    ...
    fun `email text classified as PlainText`() {
        ...
        val result = useCase.execute(data)
        // mailto 不在白名单中，fail-closed 拦截后降级为纯文本
        assertThat(result).isInstanceOf(ContentType.PlainText::class.java)
    }
    ...
    @Test
    fun `weixin link with newline injection is blocked`() {
        // 换行符(0x0A)属于控制字符范围 [0x00-0x1F]，会被正则拦截
        // "weixin\ndl/business..." 含换行符 → Blocked → PlainText
    ...
    fun `all protocols go through ValidateUrlUseCase - no bypass`() {
        // 验证：微信/支付宝协议统一经过 ValidateUrlUseCase 校验，不存在绕过路径
        val weixinData = ScannedData("weixin://dl/business/?ticket=test", 0)
        val result = useCase.execute(weixinData)
